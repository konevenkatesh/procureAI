...
def build_spec_rerank_prompt(candidates: list[dict]) -> str:
    ...
    return (
        ...
        "  \"spec_names_specific_brand_or_manufacturer\": bool   (TRUE ONLY when the spec names a SPECIFIC COMPANY or TRADE NAME — e.g. 'Polycab', 'Havells', 'Tata Steel', 'Schneider', 'Siemens', 'Crompton', 'L&T', 'Jindal', 'Kalpataru', 'Ambuja Cement', 'ACC', 'HPCL', 'BPCL', 'IOCL', 'Reliance'. CRITICAL DISTINCTIONS — these are NOT brand names, set FALSE: (a) MATERIAL NAMES — 'Aluminium', 'Steel', 'Cement', 'Concrete', 'UPVC', 'PVC', 'GI', 'CI', 'Brass', 'Stainless Steel'; (b) GENERIC FUNCTIONAL DESCRIPTORS — 'approved make', 'approved manufacturer', 'reputed manufacturer'; (c) BIS/IS/ISO STANDARD REFERENCES — 'IS:456', 'IS 1948', 'BIS Specifications', 'IS Code', 'ISO 9001'; (d) PRODUCT CATEGORIES — 'Sliding Windows', 'Cement Mortar', 'Glazed Tiles'; (e) ENGINEERING-OFFICER REFERENCES — 'Engineer-in-Charge', 'Executive Engineer'. A specific company name is one you could look up on a stock exchange or business registry — not a material, not a standard, not a category.),\n"
...
def main() -> int:
    ...
    # COMPLIANT or absence-treated-as-compliant
    if (is_compliant_l24 or is_absence) and not is_gap_violation_pre_grep:
        ...
        row = emit_verdict_row(
            ...
            value_extracted={"names_brand": names_brand,
                             "has_or_equiv": has_or_equiv,
                             "has_pac": has_pac,
                             "is_arbitrary_brand": is_arbitrary_brand,
                             "rule_shape": rule.get("shape")},
