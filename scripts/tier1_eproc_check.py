...
def main() -> int:
    ...
    if is_compliant:
        ...
        reason_label = "e_procurement_unverified_grep_fallback_retrieval_gap"
    elif is_unverified:
    ...
    if is_compliant:
        ...
        row = emit_verdict_row(
            ...
            value_extracted={"rule_shape": rule.get("shape")},
            extra_props={"violation_reason": reason_label,
                         "evidence_match_score": ev_score,
                         "evidence_match_method": ev_method,
                         "defeated": False},
        )
        print(f"  → COMPLIANT_FIRED {row['node_id']}  e-proc clause present at "
              f"line {section['line_start_local'] if section else '?'}")
        return 0
    ...
    if is_absence:
        ...
        evidence  = (f"LLM rerank top-{K} returned no e-procurement candidate, "
        ...
        print(f"  → UNVERIFIED finding (grep fallback) — retrieval missed "
              f"{len(grep_hits)} section(s) carrying e-procurement keywords")
    elif is_unverified:
    ...
    if grep_promoted_to_unverified:
        label = (
            ...
            f"section(s) with e-procurement keyword hits; requires human review"
        )
    elif is_unverified:
        label = (
    ...
    finding_props = {
        ...
        "human_review_reason": (
            ...
            f"e-procurement mandate, but exhaustive grep across {section_types} "
            f"found keyword hits in {len(grep_hits)} section(s). Reviewer should "
