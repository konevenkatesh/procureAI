...
**Forward applicability:** Any future code path that calls `_clear_kg` directly should follow the same snapshot/restore pattern (or call `build_kg` instead of mutating the DB directly). Typology scripts continue to manage their own findings via `_delete_prior_tier1_*` cleanup; that contract is unchanged. A separate concern surfaced during the Vijayawada test: kg_builder Phase 6c LLM extraction uses default `n_sections=1, max_chars=800` which on Vijayawada returns null for `estimated_value_cr` (the cost line lives in the second NIT section, ~3K chars in). After every Vijayawada rebuild the wider-window extraction (`n_sections=3, max_chars=6000`) needs to be re-run manually. Worth tightening Phase 6c's defaults in a follow-on commit, but not blocking tonight.
