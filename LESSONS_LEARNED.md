...
|--------|-----|-----|----|-----|----|----|----|--------|----|--------|----|------|-------|
| vizag | HARD 2.5% | silence | ✓ 180d | ✓ | ADV none | ✓ 5%/mo | ✓ 10% | ✓ 100% | UNV grep (L36) | ✓ 60d-post-DLP | ADV bypass (EV=null, L27) | ✓ formula M=3 | UNV kg-gap (L40) |
...
| kakinada | silence | ADV 1% | ✓ 90d | ADV absent | ADV none | ✓ §48.3 | ✓ no-MA | UNV (L35) | ✓ AP self-decl | ✓ 28d-post-DLP | HARD bypass (zero JP) | ✓ formula M=3 | UNV kg-gap (L40) |
...
**Total: 36 ValidationFindings (29 OPEN + 7 UNVERIFIED), 29 VIOLATES_RULE edges.** Thirteen typologies × six documents = seventy-eight possible finding slots: 29 OPEN violations, 7 UNVERIFIED-pending-review, 42 correctly silent. The 7 UNVERIFIED breakdown: 1 E-Proc (L35 Kakinada L24-fail) + 3 Blacklist (1 grep-fallback Vizag, 2 list-item-stitching Tirupathi/Vijayawada per L36) + 1 BG-Validity-Gap (L36 grep-fallback JA) + **2 Class-Mismatch (L40 kg_coverage_gap on Vizag NIT line 178 and Kakinada SBD line 149 — both fall outside any Section node's range; kg_builder re-build needed before promotion)**. The 2 BG-Validity-Gap GAP_VIOLATION findings on Tirupathi/Vijayawada reflect a PPP-shape knowledge-layer gap (L37). The Judicial-Preview-Bypass row is unique in the corpus: 6/6 documents trigger the violation, zero APJPA citations anywhere in 12 source markdown files (L38). The Turnover-Threshold-Excess row is the corpus's first two-shape typology (L39): 4 of 6 docs use the bid-capacity formula approach (COMPLIANT — formula approach IS the test); 2 of 6 use NREDCAP's fixed-INR turnover floor calibrated to 50%-of-total which mathematically equals 2.500× of annual contract value, just over the CVC-028 ≤2× cap. The Eligibility-Class-Mismatch row introduces the **kg_coverage_gap audit category** (L40): when both the LLM rerank and the L36 Section-bounded grep return empty, the L40 whole-file grep escalates and flags whether the matched line falls outside any Section-node range — `kg_coverage_gap=true` is a kg-build-defect signal distinct from genuine source absence.
