...
### Document Corpus (6 of 10 in KG) — Tier-1 findings across thirteen typologies

| doc_id | PBG | EMD | BV | PVC | IP | LD | MA | E-Proc | BL | BG-Val | JP | Turn | Class |
|--------|-----|-----|----|-----|----|----|----|--------|----|--------|----|------|-------|
| vizag | HARD 2.5% | silence | ✓ 180d | ✓ | ADV none | ✓ 5%/mo | ✓ 10% | ✓ 100% | UNV grep (L36) | ✓ 60d-post-DLP | ADV bypass (EV=null, L27) | ✓ formula M=3 | **ADV vague (L41 gap-fill)** |
| judicial_academy | HARD 2.5% | ADV 1% | ✓ 90d | ✓ | ADV ml-only | ✓ PCC | ✓ 10% | ✓ 100% | ✓ WB/ADB | UNV grep (23 hits) | HARD bypass (zero JP) | ✓ formula M=2 | ✓ Special exact |
| high_court | HARD 2.5% | ADV 1% | ✓ 90d | ✓ | ADV ml-only | ✓ PCC | ✓ 10% | ✓ 100% | ✓ bidder+WB | ✓ 60d-post-DLP | HARD bypass (zero JP) | ✓ formula M=2 | ✓ Special exact |
| kakinada | silence | ADV 1% | ✓ 90d | ADV absent | ADV none | ✓ §48.3 | ✓ no-MA | UNV (L35) | ✓ AP self-decl | ✓ 28d-post-DLP | HARD bypass (zero JP) | ✓ formula M=3 | **HARD class-I-vs-Special (L41 gap-fill)** |
| tirupathi | HARD 4.998% | HARD 0.998% | ✓ 180d | ADV absent | ADV ml-only | ✓ 0.1%/d | silence | ✓ 100% | UNV stitch | **GAP-VIOL 30d-post-COD (L37)** | HARD bypass (AP-GO-004) | **ADV 2.500× (128.75cr)** | silence (PPP) |
| vijayawada | HARD 5.001% | HARD 0.998% | ✓ 180d | ADV absent | ADV ml-only | ✓ 0.1%/d | silence | ✓ 100% | UNV stitch | **GAP-VIOL 30d-post-COD (L37)** | HARD bypass (AP-GO-004) | **ADV 2.500× (162.35cr)** | silence (PPP) |

**Total: 36 ValidationFindings (31 OPEN + 5 UNVERIFIED), 31 VIOLATES_RULE edges.** Thirteen typologies × six documents = seventy-eight possible finding slots: 31 OPEN violations, 5 UNVERIFIED-pending-review, 42 correctly silent. The 5 UNVERIFIED breakdown: 1 E-Proc (L35 Kakinada L24-fail) + 3 Blacklist (1 grep-fallback Vizag, 2 list-item-stitching Tirupathi/Vijayawada per L36) + 1 BG-Validity-Gap (L36 grep-fallback JA). The Class-Mismatch UNVERIFIED-kg_coverage_gap findings on Vizag and Kakinada were both promoted to OPEN by the L41 gap-filler kg_builder pass: Kakinada's "Class I & above" eligibility text at SBD L149 is now reachable via a gap-fill Section spanning L59-312, surfacing as **OPEN HARD_BLOCK class-mismatch** (Class-I floor int=5 < Special band int=6 for ECV ₹152.78cr); Vizag's vague "appropriate eligible class as per G.O.Ms.No.94" at NIT L178 is now reachable via the L13-414 gap-fill, surfacing as **OPEN ADVISORY-UNDERSPECIFIED** (clause defers to rule without naming the class). The 2 BG-Validity-Gap GAP_VIOLATION findings on Tirupathi/Vijayawada reflect a PPP-shape knowledge-layer gap (L37). The Judicial-Preview-Bypass row is unique in the corpus: 6/6 documents trigger the violation, zero APJPA citations anywhere in 12 source markdown files (L38). The Turnover-Threshold-Excess row is the corpus's first two-shape typology (L39): 4 of 6 docs use the bid-capacity formula approach (COMPLIANT — formula approach IS the test); 2 of 6 use NREDCAP's fixed-INR turnover floor calibrated to 50%-of-total which mathematically equals 2.500× of annual contract value, just over the CVC-028 ≤2× cap. The Eligibility-Class-Mismatch row introduced both the **kg_coverage_gap audit category** (L40 — whole-file grep flags missing kg_builder coverage as honest UNVERIFIED) and the **gap-filler post-process** (L41 — synthetic Section nodes for any uncovered line range >= 30 lines / 500 non-whitespace chars, automatically applied on every rebuild). Together L40 and L41 form an audit-then-fix loop: missing coverage surfaces as honest UNVERIFIED → next rebuild closes the gap → finding promotes to OPEN.
